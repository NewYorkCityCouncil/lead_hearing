...
```{r}
bar <- ggplot(data=boro_redo_formatted, aes(reorder(x=boro,per_thousand_bldngs), 
                                            y=per_thousand_bldngs,
                                            tooltip = paste0(boro, "\n", round(per_thousand_bldngs,2)," HPD Lead Litigations"))) + geom_bar(stat="identity") + 
  coord_flip() +
  geom_col_interactive(width = 0.8) +
  theme_nycc() +
  scale_y_continuous(expand = expansion(mult = c(0, .1))) +
  labs(y="HPD Lead Paint Litigations per 1000 units", x=element_blank())
